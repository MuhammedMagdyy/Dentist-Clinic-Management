  ...
  try {
    const clinic = await Clinic.findByPk(clinicId, {
      ...
      include: [
        {
          model: Specialized,
          attributes: ['id', 'examination', 'treatmentPlan'],
          include: [
            {
              model: Patient,
              attributes: ['name'],
...
exports.getInDoctor = async (req, res) => {
  try {
    const dentist = await Dentist.findAll({
      where: { worksIn: 'Specialized Clinics' },
      attributes: ['id', 'name'],
    });
    res.status(200).json({ message: 'success', data: dentist });
  } catch (err) {
...
exports.getOutDoctor = async (req, res) => {
  try {
    const dentist = await Dentist.findAll({
      where: { worksIn: 'Outpatient Clinics' },
      attributes: ['id', 'name'],
    });
    res.status(200).json({ message: 'success', data: dentist });
  } catch (err) {
...
exports.addDentist = async (req, res) => {
  const { name, email, phone, nationalId, worksIn } = req.body;
  try {
...
exports.editDentist = async (req, res) => {
  ...
  try {
    ...
    dentist.email = email;
    dentist.phone = phone;
    dentist.nationalId = nationalId;
    dentist.worksIn = worksIn;
